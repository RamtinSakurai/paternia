...
type DrawFn = (ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) => void;
...
function clipCircle(ctx: CanvasRenderingContext2D, s: number) {
  ctx.beginPath();
  ctx.arc(s / 2, s / 2, s / 2 - 2, 0, Math.PI * 2);
  ctx.clip();
}

function fillBg(ctx: CanvasRenderingContext2D, s: number, color: string) {
  ctx.fillStyle = color;
...
// ============================================================
function drawFractal(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
  ...
  const drawBranch = (x: number, y: number, angle: number, len: number, depth: number, colorIdx: number) => {
...
// ============================================================
function drawKaleidoscope(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
...
// ============================================================
function drawEscher(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawCrystal(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawSubway(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawSeigaiha(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawFireworks(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawArtDeco(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawParade(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
...
// ============================================================
function drawExpressionism(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
...
// ============================================================
function drawMycelium(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
  ...
  const drawMycelBranch = (x: number, y: number, angle: number, length: number, depth: number) => {
...
// ============================================================
function drawMelting(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
...
// ============================================================
function drawWarning(ctx: CanvasRenderingContext2D, s: number, scores: Scores, _noise: NoiseFn, t: number) {
...
// ============================================================
function drawRipple(ctx: CanvasRenderingContext2D, s: number, scores: Scores, noise: NoiseFn, t: number) {
...
// Pattern mapping
// ============================================================
const PATTERN_MAP: Record<Axis, Partial<Record<Axis, DrawFn>>> = {
  O: { C: drawFractal, E: drawKaleidoscope, A: drawSumiE, N: drawEscher },
  C: { O: drawCrystal, E: drawSubway, A: drawSeigaiha, N: drawSeismograph },
  E: { O: drawFireworks, C: drawArtDeco, A: drawParade, N: drawExpressionism },
  A: { O: drawMycelium, C: drawKnit, E: drawSunshine, N: drawPetals },
  N: { O: drawMelting, C: drawStipple, E: drawWarning, A: drawRipple },
};

function pickDrawFn(scores: Scores): DrawFn {
  const axes = (Object.keys(scores) as Axis[])
    .map((a) => ({ axis: a, dev: Math.abs(scores[a] - 0.5) }))
    .sort((a, b) => b.dev - a.dev);
  const primary = axes[0].axis;
  const secondary = axes[1].axis;
  return PATTERN_MAP[primary][secondary] || drawFractal;
}

// ============================================================
// Component
// ============================================================
export default function PatternCanvas({ scores, size = 320, className }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animRef = useRef<number>(0);
  const pulseRef = useRef<number>(0); // 0 = no pulse, 1 = just tapped, decays
  ...
  useEffect(() => {
    ...
    const seed = Math.round(
      scores.O * 1000 + scores.C * 777 + scores.E * 333 + scores.A * 111 + scores.N * 55
    );
    const noise = createNoise(seed);
    let t = 0;

    const drawFn = pickDrawFn(scores);
    ...
    const draw = () => {
      ...
      drawFn(ctx, size, scores, noise, t);

      ctx.restore();

      // Circle border (pulse-reactive)
      const pulseAmt = pulseRef.current;
      ctx.strokeStyle = `rgba(255,255,255,${0.08 + pulseAmt * 0.3})`;
      ctx.lineWidth = 1 + pulseAmt * 3;
      ctx.beginPath();
      ctx.arc(size / 2, size / 2, size / 2 - 1, 0, Math.PI * 2);
      ctx.stroke();
